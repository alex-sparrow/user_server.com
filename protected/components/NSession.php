...
class NSession{
    ...
    public function check($s_key){
        ...
        if(empty($c_ses)) return false; //exit if no session
    ...
    public function set($u_id){
        ...
        if(empty($c_ses))  $c_ses = new SessionId();   //set if no session
        $c_ses->session_key = $this->gen_key();
        $c_ses->exp_date = strtotime('+ 1 day');
        $c_ses->user_id = $u_id;
        $c_ses->save();
        $this->s_key = $c_ses->session_key;
        return $c_ses->session_key;
    }
    //get session key
    function get_key(){
        return $this->s_key;
